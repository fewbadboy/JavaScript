...
let com3 = /^((10\.((1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d)\.){2}(1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d))|(172\.(1[6-9]|2\d|3[0-1])\.(1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d)\.(1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d))|(192\.168\.(1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d)\.(1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d)))$/
let com2 = [
...
let A = /^((12[0-6]|1[0-1]\d|[1-9]\d|[1-9])\.)((1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d)\.){2}(1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d)$/ // A类地址1.0.0.0~126.255.255.255;
// let B = /^(12[8-9]|1[3-8][0-9]|19[0-1])(\.(2[0-4][0-9]|25[0-5]|1[0-9]{2}|[1-9][0-9]|[0-9])){3}$/ // B类地址128.0.0.0~191.255.255.255;
let B = /^(12[8-9]|1[3-8]\d|19[0-1])\.((1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d)\.){2}(1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d)$/ // B类地址128.0.0.0~191.255.255.255;
// let C = /^(19[2-9]|2[0-1][0-9]|22[0-3])(\.(2[0-4][0-9]|25[0-5]|1[0-9]{2}|[1-9][0-9]|[0-9])){3}$/ // C类地址192.0.0.0~223.255.255.255;
let C = /^(19[2-9]|2[0-1]\d|22[0-3])\.((1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d)\.){2}(1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d)$/ // C类地址192.0.0.0~223.255.255.255;
// let D = /^(22[4-9]|23[0-9])(\.(2[0-4][0-9]|25[0-5]|1[0-9]{2}|[1-9][0-9]|[0-9])){3}$/ // D类地址224.0.0.0~239.255.255.255；
let D = /^(22[4-9]|23\d)\.((1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d)\.){2}(1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d)$/ // D类地址224.0.0.0~239.255.255.255；
// let E = /^(24[0-9]|25[0-5])(\.(2[0-4][0-9]|25[0-5]|1[0-9]{2}|[1-9][0-9]|[0-9])){3}$/ // E类地址240.0.0.0~255.255.255.255
let E = /^(24\d|25[0-5])\.((1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d)\.){2}(1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d)$/ // E类地址240.0.0.0~255.255.255.255
// let F = /^(0|127)(\.(2[0-4][0-9]|25[0-5]|1[0-9]{2}|[1-9][0-9]|[0-9])){3}$/ // 0.*.*.*.* 和127.*.*.*  不输入上述任何一类
let F = /^((0|127)\.((1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d)\.){2}(1\d\d|2[0-4]\d|25[0-5]|[1-9]\d|\d))$/ // 0.*.*.*.* 和127.*.*.*  不输入上述任何一类
...
while (i < rlen) {
    ...
    if (arr[0].search(G) !== -1) { // 合法IP
        if (arr[0].search(F) !== -1) {
            ...
            if(arr[0].search(com3) !== -1) {
                result[6] += 1
            }

            for (let i = 0; i < 5; i++) {
                if (arr[0].search(com1[0]) !== -1) {
                    console.log(arr[0])
                }
